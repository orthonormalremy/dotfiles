...
if [ -z "$(aws ec2 describe-key-pairs --filters "Name=key-name,Values=id_rsa" --output text)" ]; then
...
aws ec2 run-instances \
    ...
    --key-name "id_rsa" \
    --block-device-mappings '{"DeviceName":"/dev/xvda","Ebs":{"Encrypted":false,"DeleteOnTermination":true,"Iops":3000,"VolumeSize":512,"VolumeType":"gp3","Throughput":125}}' \
